...
if [ "true" == "$QUEUE" ]; then
  ...
  nohup php artisan queue:listen --tries=3 --timeout=120> /dev/null 2>&1 &
  nohup php artisan queue:listen --queue=sms --tries=3 --timeout=60 > /dev/null 2>&1 &
  nohup php artisan queue:listen --queue=notification --tries=3 --timeout=60 > /dev/null 2>&1 &
  nohup php artisan queue:listen --queue=cron --tries=3 > /dev/null 2>&1 &
  nohup php artisan queue:listen --queue=sync_contacts --tries=3 --timeout=60 > /dev/null 2>&1 &
fi
